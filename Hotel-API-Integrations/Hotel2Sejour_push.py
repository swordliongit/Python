...
def create_email_item(username, dbname, uid, emailtype, emailaddr):
    return {
        "UserName": username,
        "DatabaseName": dbname,
        "UniqueID": uid,
...
def create_delete_item(username, tablename, uid, deletedate, dbname):
    return {
        "UserName": username,
        "TableName": tablename,
        "UniqueID": uid,
...
def create_hotel_item(uid, hotel, hotelname, region, pax, dbname, username):
    return {
        "UniqueID": uid,
        "Hotel": hotel,
        "HotelName": hotelname,
...
def H2S_init():
    ...
    reservation_item = create_reservation_item(
        opcode="ZTour",  # Sisteminizdeki rezervasyonun operator kodudur
        opname="ZEN TOUR TURIZM A.Ş. ANKARA",  # Sisteminizdeki rezervasyonun operator adıdır
        ...
        cindate=datetime(2023, 9, 2).strftime("%Y.%m.%d"),  # Otel giriş tarihi
        coutdate=datetime(2023, 9, 4).strftime("%Y.%m.%d"),  # Otel çıkış tarihi
        ...
        sellingdate=datetime(2023, 9, 2).strftime(
            "%Y.%m.%d"
        ),  # Rezervasyon Satış tarihi
        allotmenttype="G",  # Rezervasyon Kontenjan türü (N : Normal , G : Guarantee , S : OnRequest)
        resuser="ARTIN",  # Rezervasyonu yapan kullanıcı kodu
        ...
        chgdate=datetime(2023, 9, 4).strftime(
            "%Y.%m.%d"
        ),  # Kaydın sisteminizdeki en son değişiklik tarihi
        username="ARTIN_TP",  # Hotel2Sejour sistemi tarafından verilen servis kullanıcı adı
    )
    customer_item = create_customer_item(
        ...
        arrivaltime=datetime(2023, 9, 16).strftime(
            "%m.%d.%Y"
        ),  # Müşteri geliş tarih ve saati
        departure="CAI041",  # Müşteri dönüş yeri Örn : TK 456
        departuretime=datetime(2023, 9, 25).strftime(
            "%m.%d.%Y"
        ),  # Müşteri dönüş tarihi ve numarası
        dbname="ARTINSYSTEMS",  # Hotel2Sejour tarafından verilir
        chgdate=datetime(2023, 9, 25).strftime(
            "%m.%d.%Y"
        ),  # Kaydın sisteminizdeki en son değişiklik tarihi
    ...
    extra_item = create_extra_item(
        ...
        resorder=1,  # Sisteminizdeki rezervasyon ResNo ‘sunun sırasıdır
        dbname="ARTINSYSTEMS",  # Hotel2Sejour tarafından verilir
        chgdate=datetime(2023, 9, 16).strftime(
            "%m.%d.%Y"
        ),  # Kaydın sisteminizdeki en son değişiklik tarihi
    ...
    delete_item = create_delete_item(
        ...
        deletedate=datetime(2023, 9, 17).strftime(
            "%m.%d.%Y"
        ),  # Sisteminizdeki kaydın silinme tarihini verir
        dbname="ARTINSYSTEMS",  # Hotel2Sejour tarafından verilir
    )
